...
void eval_b_splines_struphy(
  ...
  std::vector<double> left(pn, 0);
  std::vector<double> right(pn, 0);
...
void eval_d_splines_struphy(
  ...
  std::vector<double> left(pn - 1, 0);
  std::vector<double> right(pn - 1, 0);

  // Compute B-splines up to degree p-1
  std::vector<double> b_values(pn, 0);
  ...
  for (size_t il = 0; il < pd + 1; ++il) {
    values[pd - il] = pn / (tn[span - il + pn] - tn[span - il]) * b_values[pd - il];
  }
}

void eval_b_d_splines_struphy(
    const std::vector<double>& tn,
    int pn,
    double eta,
    int span,
