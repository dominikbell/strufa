...
$$\left\{x_i\right\}_{i=0,\ldots,n-1+p} = \left\{ - p \Delta x, - (p-1) \Delta x, \ldots, - \Delta x, 0, \Delta x, 2 \Delta x, \ldots, (n-1) \Delta x = L - \Delta x \right\}$$

The basis functions (B-splines) of degree $p=0$ are defined as
$$N^0_i(x) = \begin{cases}
1 & \text{if } x \in [x_i, x_{i+1}[ \\
0 & \text{else}
\end{cases} \qquad i = 0, \ldots, n-1$$
